...
def draw():
    glClearColor(1, 1, 1, 1)  # SECTION 2 -> make background white
    glClear(GL_COLOR_BUFFER_BIT)  # SECTION 2 -> clear the buffer
    glLineWidth(2)  # -> the width of the line
    glColor3f(0, 0, 0)
    #########################
    # draw_circle(r=.8, st_theta=-30, end_theta=20)  # TODO: Try this first and comment the other parts
    #########################
    ...
    draw_circle(r=.2, xc=0, yc=-.3, st_theta=180, end_theta=360)  # mouth, # TODO: Try reversing -180 and 0
    #########################
    glFlush()
