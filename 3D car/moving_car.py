...
def init_projection_Q3():  # Q3
    ...
    glLoadIdentity()
    glOrtho(-6, 6, -6, 6, 6, -6)  # TODO : replace those 5's with 3
...
def draw():
    ...
    glTranslate(shift, 0, 0)  # to move all poin
    glScale(4, 1, 2)
    glutWireCube(1)
    # # #########################################
    # # ########## top cube
    glLoadIdentity()
    reposition_camera_Q3()  # Q3
    glTranslate(shift, 0, 0)  # the first to move the car
    glTranslate(0, 1 - .15, 0)  # to set cube in right position.
    glScale(2, .7, 1.5)
    glutWireCube(1)
    # # #########################################
    # # ############# wheels
    ...
    shift = shift + (0.01 if forward else -.01)  # Q3
